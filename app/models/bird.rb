class Bird < ApplicationRecord
  ...
  # TAXONOMIC FAMILIES constant contains names of all taxonomic family names from the
  # Audubon Society's North American Bird Guide formatted as url-safe strings.
  ...
  # add_url sets the url attribute of a bird to its corresponding page in the guide
  def add_url
    self.update(url: self.audubon_url)
  end 

  # audubon_url returns the url of a bird's corresponding page in the guide
  def audubon_url
    "https://www.audubon.org/field-guide/bird/" + self.url_safe_attribute("common_name")
  end

  def self.taxonomic_families
    TAXONOMIC_FAMILIES
  end
