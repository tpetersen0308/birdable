...
class ExercisePage extends Component {
  ...
  render() {
    ...
    const familyMenuItems = taxonomicFamilies.map(family => <option value={family} onSelect={this.addFamilySelection}>{family}</option>)
    const regionMenuItems = regions.map(region => <option value={region} onSelect={this.addRegionSelection}>{region}</option>)
